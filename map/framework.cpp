...
//    scale = scales::GetUpperComfortScale();
  ...
  ActivateMapSelection(true, df::SelectionShape::OBJECT_USER_MARK, info, false);
...
void Framework::ShowSearchResult(search::Result const & res)
{
  ...
  ActivateMapSelection(false, df::SelectionShape::OBJECT_POI, info, false);
...
void Framework::CreateDrapeEngine(ref_ptr<dp::OGLContextFactory> contextFactory, DrapeCreationParams && params)
{
  ...
  {
      if (m_routingSession.GetTargetIndex() > 0)
      {
  ...
  {
    place_page::Info info;
    ActivateMapSelection(false, OnTapEventImpl(*m_lastTapEvent, info), info, (*m_lastTapEvent).m_isLong);
  }
}
  ...
  if (result != FAILED)
  {
    ...
    if (result != NO_NEED_CLICK)
    {
      ...
      if (apiMark)
      {
        ...
        ActivateMapSelection(false, df::SelectionShape::OBJECT_USER_MARK, info, false);
      }
      else
      {
                                     ...
                                     place_page::Info const & info, bool isLong) const
{
  ...
  if (m_activateMapSelectionFn)
    m_activateMapSelectionFn(info, isLong);
  else
...
void Framework::UpdatePlacePageInfoForCurrentSelection()
{
  ...
  place_page::Info info;
  ActivateMapSelection(false, OnTapEventImpl(*m_lastTapEvent, info), info, false);
}
...
void Framework::OnTapEvent(df::TapInfo const & tapInfo)
{
  ...
  if (selection != df::SelectionShape::OBJECT_EMPTY)
  {
    ...
    ActivateMapSelection(true, selection, info, tapInfo.m_isLong);
  }
  else
  {
